...
void OssManager::SignalCallback(struct ev_loop* loop, struct ev_signal* watcher, int revents)
{
...
void OssManager::WaitToExitTaskCallback(struct ev_loop* loop, struct ev_timer* watcher, int revents)
{
    if (watcher->data != NULL)
    {
...
bool OssManager::ManagerTerminated(struct ev_signal* watcher)
{
...
bool OssManager::RecvDataAndDispose(tagManagerIoWatcherData* pData, struct ev_io* watcher)
{
    ...
    else
    {
        ...
        if (iReadLen > 0)
        {
            ...
            {
                LOG4_TRACE("conn_iter->second->pRecvBuff->ReadableBytes() = %d",
                                conn_iter->second->pRecvBuff->ReadableBytes());
                ...
                bool bResult = oSwitchMsgHead.ParseFromArray(
                                  conn_iter->second->pRecvBuff->GetRawReadBuffer(), gc_uiMsgHeadSize);
                if (bResult)
                {
                    MsgHead oInMsgHead;
...
bool OssManager::SendTo(const tagMsgShell& stMsgShell, const MsgHead& oMsgHead, const MsgBody& oMsgBody)
{
    ...
    else
    {
        if (iter->second->ulSeq == stMsgShell.ulSeq)
        {
            ...
            int iWriteIdx = iter->second->pSendBuff->GetWriteIndex();
            iWriteLen = iter->second->pSendBuff->Write(oSwitchMsgHead.SerializeAsString().c_str(), oSwitchMsgHead.ByteSize());
...
void OssManager::CreateWorker()
{
    ...
    for (unsigned int i = 0; i < m_uiWorkerNum; ++i)
    {
        ...
        if (iPid == 0)   // 子进程
        {
            ...
            OssWorker worker(m_strWorkPath, iControlFds[1], iDataFds[1], i, m_oCurrentConf);
            worker.Run();
            exit(-2);
        }
        else if (iPid > 0)   // 父进程
...
//            AddIoErrorEvent(iControlFds[0]);
            AddIoReadEvent(iDataFds[0]);
//            AddIoErrorEvent(iDataFds[0]);
        }
        else
        {
            LOG4_ERROR("error %d: %s", errno, strerror_r(errno, m_pErrBuff, 1024));
...
bool OssManager::RestartWorker(int iDeathPid)
{
    ...
    if (worker_iter != m_mapWorker.end())
    {
        ...
        if (iNewPid == 0)   // 子进程
        {
            ...
            x_sock_set_block(iControlFds[1], 0);
            x_sock_set_block(iDataFds[1], 0);
            OssWorker worker(m_strWorkPath, iControlFds[1], iDataFds[1], iWorkerIndex, m_oCurrentConf);
            worker.Run();
...
bool OssManager::AddWaitToExitTaskEvent(const tagMsgShell& stMsgShell,uint32 cmd,uint32 seq)
{
    LOG4_DEBUG("%s()", __FUNCTION__);
...
const std::map<int, tagWorkerAttr>& OssManager::GetWorkerAttr() const
{
	return(m_mapWorker);
}
...
bool OssManager::RestartWorkers()
{
    LOG4_TRACE("%s()", __FUNCTION__);
    std::map<int, tagWorkerAttr>::iterator worker_iter;
    for (worker_iter = m_mapWorker.begin();
...
bool OssManager::CheckWorkerLoadNullExit(tagManagerWaitExitWatcherData* pData,struct ev_timer* watcher)
{
    std::map<int, tagWorkerAttr>::iterator worker_iter = m_mapWorker.begin();
...
bool OssManager::CheckWorkerLoadNullRestartWorkers(tagManagerWaitExitWatcherData* pData,struct ev_timer* watcher)
{
    std::map<int, tagWorkerAttr>::iterator worker_iter = m_mapWorker.begin();
        ...
        if (worker_conn_iter != m_mapFdAttr.end())
        {
            ...
            if (iWriteLen < 0)
            {
                ...
                else if (EAGAIN == iErrno)  // 内容未写完，添加或保持监听fd写事件
                {
                    ...
                    AddIoWriteEvent(worker_conn_iter->first);
                }
            }
            else if (iWriteLen > 0)
            {
...
bool OssManager::SendToWorkerWithMod(unsigned int uiModFactor,const MsgHead& oMsgHead, const MsgBody& oMsgBody)
{
    int iErrno = 0;
    int iWriteLen = 0;
    int iNeedWriteLen = 0;
        ...
        if (oConnWorker.ParseFromString(oInMsgBody.body()))
        {
                            ...
                            worker_iter != m_mapWorker.end(); ++worker_iter)
            {
                if (oConnWorker.worker_index() == worker_iter->second.iWorkerIndex)
                {
                    ...
                    int iErrno = send_fd_with_attr(worker_iter->second.iDataFd, stMsgShell.iFd, szIp, 16, loss::CODEC_PROTOBUF);
