...
bool NodeManager::SendTo(const tagMsgShell& stMsgShell)
{
...
bool NodeManager::SendTo(const tagMsgShell& stMsgShell, const MsgHead& oMsgHead, const MsgBody& oMsgBody)
{
...
bool NodeManager::SetConnectIdentify(const tagMsgShell& stMsgShell, const std::string& strIdentify)
{
    LOG4_TRACE("%s()", __FUNCTION__);
    std::map<int, tagConnectionAttr*>::iterator iter = m_mapFdAttr.find(stMsgShell.iFd);
    if (iter == m_mapFdAttr.end())
...
bool NodeManager::AddWaitToExitTaskEvent(const tagMsgShell& stMsgShell,uint32 cmd,uint32 seq)
{
    LOG4_DEBUG("%s()", __FUNCTION__);
...
bool NodeManager::AddIoReadEvent(int iFd)
{
...
bool NodeManager::AddIoWriteEvent(int iFd)
{
    LOG4_TRACE("%s(fd %d)", __FUNCTION__, iFd);
    ev_io* io_watcher = NULL;
    std::map<int, tagConnectionAttr*>::iterator iter =  m_mapFdAttr.find(iFd);
...
bool NodeManager::RemoveIoWriteEvent(int iFd)
{
    LOG4_TRACE("%s(fd %d)", __FUNCTION__, iFd);
    ev_io* io_watcher = NULL;
    std::map<int, tagConnectionAttr*>::iterator iter =  m_mapFdAttr.find(iFd);
...
bool NodeManager::DisposeDataFromWorker(const tagMsgShell& stMsgShell, const MsgHead& oInMsgHead, const MsgBody& oInMsgBody, thunder::CBuffer* pSendBuff)
{
...
bool NodeManager::DisposeDataAndTransferFd(const tagMsgShell& stMsgShell, const MsgHead& oInMsgHead, const MsgBody& oInMsgBody, thunder::CBuffer* pSendBuff)
{
    LOG4_DEBUG("%s(cmd %u, seq %u)", __FUNCTION__, oInMsgHead.cmd(), oInMsgHead.seq());
    int iErrno = 0;
    ConnectWorker oConnWorker;
...
bool NodeManager::DisposeDataFromCenter(
                ...
                thunder::CBuffer* pWaitForSendBuff)
{
    LOG4_DEBUG("%s(cmd %u, seq %u)", __FUNCTION__, oInMsgHead.cmd(), oInMsgHead.seq());
    int iErrno = 0;
