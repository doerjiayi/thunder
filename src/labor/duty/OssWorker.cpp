            ...
            while (conn_iter->second->pRecvBuff->ReadableBytes() >= gc_uiClientMsgHeadSize)
            {
                ...
                if (eConnectStatus_init == conn_iter->second->ucConnectStatus)
                {
                    ...
                    if (CODEC_STATUS_ERR == eCodecStatus &&
                        (loss::CODEC_WEBSOCKET_EX_JS == conn_iter->second->eCodecType || loss::CODEC_WEBSOCKET_EX_PB == conn_iter->second->eCodecType))
                    {
                        //切换为http协议
                        LOG4_DEBUG("failed to decode for codec %d,switch to CODEC_HTTP",conn_iter->second->eCodecType);
                        conn_iter->second->eCodecType = loss::CODEC_HTTP;
                        std::map<loss::E_CODEC_TYPE, StarshipCodec*>::iterator codec_iter = m_mapCodec.find(conn_iter->second->eCodecType);
                        if (codec_iter == m_mapCodec.end())
                        {
                            LOG4_ERROR("no codec found for %d!", conn_iter->second->eCodecType);
                    ...
                    if (CODEC_STATUS_ERR == eCodecStatus && loss::CODEC_HTTP == conn_iter->second->eCodecType)
                    {
                        ...
                        conn_iter->second->eCodecType = loss::CODEC_PRIVATE;
                        std::map<loss::E_CODEC_TYPE, StarshipCodec*>::iterator codec_iter = m_mapCodec.find(conn_iter->second->eCodecType);
                        if (codec_iter == m_mapCodec.end())
                        {
                            LOG4_ERROR("no codec found for %d!", conn_iter->second->eCodecType);
                ...
                if (CODEC_STATUS_OK == eCodecStatus)
                {
                    ...
                    if (oInMsgHead.cmd() > 0)
                    {
                        ...
                        else
                        {
                            ...
                            if (loss::CODEC_WEBSOCKET_EX_PB == conn_iter->second->eCodecType ||
                                            loss::CODEC_WEBSOCKET_EX_JS == conn_iter->second->eCodecType)
                            {
...
Session* OssWorker::GetSession(uint64 uiSessionId, const std::string& strSessionClass)
{
...
bool OssWorker::Init(loss::CJsonObject& oJsonConf)
{
    ...
    pCodec = new CodecWebSocketJson(loss::CODEC_WEBSOCKET_EX_JS);
    pCodec->SetLogger(m_oLogger);
    m_mapCodec.insert(std::pair<loss::E_CODEC_TYPE, StarshipCodec*>(loss::CODEC_WEBSOCKET_EX_JS, pCodec));
    pCodec = new CodecWebSocketPb(loss::CODEC_WEBSOCKET_EX_PB);
    pCodec->SetLogger(m_oLogger);
    m_mapCodec.insert(std::pair<loss::E_CODEC_TYPE, StarshipCodec*>(loss::CODEC_WEBSOCKET_EX_PB, pCodec));
...
void OssWorker::DelMsgShell(const std::string& strIdentify,const tagMsgShell& stMsgShell)
{
    std::map<std::string, tagMsgShell>::iterator shell_iter = m_mapMsgShell.find(strIdentify);
    if (shell_iter == m_mapMsgShell.end())
    {
...
void OssWorker::GetNodeIdentifys(const std::string& strNodeType, std::vector<std::string>& strIdentifys)
{
    strIdentifys.clear();
...
bool OssWorker::ParseFromMsg(const MsgBody& oInMsgBody,google::protobuf::Message &message)
{
...
bool OssWorker::SendToClient(const tagMsgShell& stMsgShell,MsgHead& oMsgHead,const google::protobuf::Message &message,
...
bool OssWorker::SendToClient(const std::string& strIdentify,MsgHead& oMsgHead,const google::protobuf::Message &message,
                const std::string& additional,uint64 sessionid,const std::string& strSession)
{
...
bool OssWorker::BuildClientMsg(MsgHead& oMsgHead,MsgBody &oMsgBody,const google::protobuf::Message &message,
                            const std::string& additional,uint64 sessionid,const std::string& strSession)
{
    oMsgBody.Clear();
    {
...
bool OssWorker::SendTo(const tagMsgShell& stMsgShell, const HttpMsg& oHttpMsg, HttpStep* pHttpStep)
{
    ...
    else
    {
        if (conn_iter->second->ulSeq == stMsgShell.ulSeq)
        {
            ...
            else if(loss::CODEC_WEBSOCKET_EX_JS == conn_iter->second->eCodecType)
            {
                if (conn_iter->second->pWaitForSendBuff->ReadableBytes() > 0)   // 正在连接
                {
...
std::string OssWorker::GetConnectIdentify(const tagMsgShell& stMsgShell)
{
    LOG4_TRACE("%s()", __FUNCTION__);
    std::map<int, tagConnectionAttr*>::iterator iter = m_mapFdAttr.find(stMsgShell.iFd);
    if (iter == m_mapFdAttr.end())
