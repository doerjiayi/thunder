...
sphinx_client * sphinx_create ( sphinx_bool copy_args)
{
	...
	client->host					= strchain ( client, "localhost" );//地址（双向链表内存分配）
	client->port					= 9312;//端口
	client->timeout					= 0.0f;
	client->offset					= 0;
	client->limit					= 20;
	client->mode					= SPH_MATCH_EXTENDED2;//拓展匹配模式--使用排序模式
	client->num_weights				= 0;
	client->weights					= NULL;
	client->sort					= SPH_SORT_RELEVANCE;//按相关度降序排列
	client->sortby					= NULL;
	client->minid					= 0;
	client->maxid					= 0;
