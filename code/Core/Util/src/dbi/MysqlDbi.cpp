...
void StringReplace(string &str, const string &old_value,const string &new_value)
{
...
int util::CMysqlDbi::InitDbConn(const util::tagDbConfDetail& stDbConfDetail)
{
	FreeResult();
	InitVariables();
...
int util::CMysqlDbi::SetMysqlOptions(
...
int util::CMysqlDbi::MysqlRealConnect(
        const char* szIp,
        const char* szUserName,
        const char* szUserPwd,
        const char* szDbName,
...
int util::CMysqlDbi::SetDbConnCharacterSet(const char* szCharacterSet)
{
    int ret = 0;
...
int util::CMysqlDbi::EscapeString(
        char* szTo,
        const char* szFrom,
...
int util::CMysqlDbi::MysqlPing()
{
    return mysql_ping(&m_dbInstance);
}

int util::CMysqlDbi::SetAutoCommit(my_bool mode)
{
    int ret = 0;
...
int util::CMysqlDbi::ExecSql(const std::string& strSql)
{
...
unsigned int util::CMysqlDbi::AffectRows()
{
	return(mysql_affected_rows(&m_dbInstance));
}

const MYSQL_RES* util::CMysqlDbi::StoreResult()
{
...
const MYSQL_RES* util::CMysqlDbi::UseResult()
{
	if (m_iErrno != 0)  // 查询是失败的
	{
	    return NULL;
...
int util::CMysqlDbi::GetResultSet(T_vecResultSet& vecResultSet)
{
	vecResultSet.clear();
	if (m_iQueryResult == 0)
    {
...
int util::CMysqlDbi::GetResultToFile()
{
	 //查询失败，取不了结果集
	 if (m_iQueryResult != 0)
	 {
	  ...
	  {
		  //默认会清空文件，刚好是需要的
		  ofstream fOut(m_strTempFileName.c_str());
		  if (!fOut.good())
		  {
			  return -1;
		  }
...
util::T_mapRow& util::CMysqlDbi::GetRowFromFile()
{
	m_mapRow.clear();
...
int util::CMysqlDbi::GetResultDict(T_vecDict& vecDict)
{
	m_vecDict.clear();
...
int util::CMysqlDbi::GetResultRow(T_mapRow& mapRow)
{
	mapRow.clear();
...
MYSQL_ROW util::CMysqlDbi::GetRow()
{
    if (m_pQueryRes == NULL)
    {
        return NULL;
...
unsigned long* util::CMysqlDbi::FetchLengths()
{
    if (m_pQueryRes != NULL)
    {
...
unsigned int util::CMysqlDbi::FetchFieldNum()
{
    if (m_pQueryRes != NULL)
    {
...
MYSQL_FIELD* util::CMysqlDbi::FetchFields()
{
    if (m_pQueryRes != NULL)
    {
...
unsigned int util::CMysqlDbi::FetchFieldCount()
{
    return mysql_field_count(&m_dbInstance);
}

void util::CMysqlDbi::FreeResult()
{
    if (m_pQueryRes != NULL)
    {
...
const char* util::CMysqlDbi::MysqlInfo()
{
    return mysql_info(&m_dbInstance);
}

void util::CMysqlDbi::InitVariables()
{
	m_iQueryResult = 0;
	m_iErrno = 0;
	m_pQueryRes = NULL;
...
bool util::CMysqlDbi::CleanBeforeExecSql()
{
	//测试数据库连接的可用性
