...
class CMysqlDbi : public util::CDbi
{
public:
    CMysqlDbi();
    CMysqlDbi(
    ...
    //初始化连接实例
    virtual int InitDbConn(const util::tagDbConfDetail& stDbConfDetail);
...
public:
     ...
     * util::CMysqlDbi::InitDbConn(const util::tagDbConfDetail& stDbConfDetail)
     * 中的使用。
     */
    ...
    const tagDbConfDetail& GetDbConf(){return m_stDbConfDetail;}
    const char* MysqlInfo();

    virtual int GetResultToFile();
    util::T_mapRow& GetRowFromFile();
...
private:
    ...
    util::T_vecDict m_vecDict;	//结果集数据字典
    util::T_mapRow m_mapRow;	//结果集行

    util::tagDbConfDetail m_stDbConfDetail; // 数据库连接信息
 };
