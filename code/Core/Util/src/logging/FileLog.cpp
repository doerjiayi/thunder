...
util::CLog::CLog(const std::string strLogFile, int iLogLev,
...
int util::CLog::OpenLogFile(const std::string strLogFile)
{
...
int util::CLog::WriteLog(int iLev, const char* szLogStr, ...)
{
    if (iLev > m_iLogLevel)
    {
        return 0;
...
int util::CLog::Vappend(int iLev, const char* szLogStr, va_list ap)
{
    long file_size = -1;
    if (NULL != m_fp)
    {
    ...
    fprintf(m_fp, "[%s] [%s]\t", util::GetCurrentTime(20).c_str(),
       util::LogLevMsg[iLev].c_str());
