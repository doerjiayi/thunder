...
class CLog : public util::CLogBase
{
public:
    explicit CLog(
            const std::string strLogFile,
            int iLogLev = util::DEFAULT_LOG_LEVEL,
            unsigned int uiMaxFileSize = util::gc_uiMaxLogFileSize,
            unsigned int uiMaxRollFileIndex = util::gc_uiMaxRollLogFileIndex);
    virtual ~CLog()
    {
        fclose(m_fp);
    }
