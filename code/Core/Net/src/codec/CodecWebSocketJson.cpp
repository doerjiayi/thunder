...
//http parse
int CodecWebSocketJson::OnMessageBegin(http_parser *parser)
{
...
int CodecWebSocketJson::OnUrl(http_parser *parser, const char *at, size_t len)
{
    HttpMsg* pHttpMsg = (HttpMsg*) parser->data;
...
int CodecWebSocketJson::OnStatus(http_parser *parser, const char *at, size_t len)
{
    HttpMsg* pHttpMsg = (HttpMsg*) parser->data;
...
int CodecWebSocketJson::OnHeaderField(http_parser *parser, const char *at,
                size_t len)
{
    HttpMsg* pHttpMsg = (HttpMsg*) parser->data;
...
int CodecWebSocketJson::OnHeaderValue(http_parser *parser, const char *at,
                size_t len)
{
    HttpMsg* pHttpMsg = (HttpMsg*) parser->data;
...
int CodecWebSocketJson::OnHeadersComplete(http_parser *parser)
{
...
int CodecWebSocketJson::OnBody(http_parser *parser, const char *at, size_t len)
{
    HttpMsg* pHttpMsg = (HttpMsg*) parser->data;
...
int CodecWebSocketJson::OnMessageComplete(http_parser *parser)
{
    HttpMsg* pHttpMsg = (HttpMsg*) parser->data;
...
int CodecWebSocketJson::OnChunkHeader(http_parser *parser)
{
    return (0);
}

int CodecWebSocketJson::OnChunkComplete(http_parser *parser)
{
    return (0);
}

CodecWebSocketJson::CodecWebSocketJson(util::E_CODEC_TYPE eCodecType,
                const std::string& strKey)
                : StarshipCodec(eCodecType, strKey)
{
}

CodecWebSocketJson::~CodecWebSocketJson()
{
}
                ...
                const MsgBody& oMsgBody, util::CBuffer* pBuff)
{
    ...
    int iHadWriteLen = 0;
    int iWriteLen = 0;
    LOG4_TRACE("cmd %u, seq %u, msgbody_len %u", oMsgHead.cmd(), oMsgHead.seq(),
...
//    else
    {
        ...
        LOG4_DEBUG("strJsonMsg(%u,%s)",strJsonMsg.size(),strJsonMsg.c_str());
        /*
         * body为BYTE需要编码
        ...
        {//payload数据
            ...
            if(strJsonMsg.size() > 0)
            {
                ...
                {
                    if (!Zip(strJsonMsg, strCompressData))
                    {
                ...
                {
                    if (!Gzip(strJsonMsg, strCompressData))
                    {
                ...
                if (gc_uiRc5Bit & oMsgHead.cmd())   //加密
                {
                    ...
                    {
                        if (!Rc5Encrypt(strJsonMsg, strEncryptData))
                        {
                            LOG4_ERROR("Rc5Encrypt error!");
                            return (CODEC_STATUS_ERR);
                        }
        ...
        {
            iNeedWriteLen = strJsonMsg.size();
            iWriteLen = pBuff->Write(strJsonMsg.c_str(), strJsonMsg.size());
        }
        if (iWriteLen != iNeedWriteLen)
        {
                ...
                util::CBuffer* pBuff)
{
    ...
    if (!oHttpMsg.http_major() && !oHttpMsg.http_minor()) //1.1
    {
...
E_CODEC_STATUS CodecWebSocketJson::Decode(util::CBuffer* pBuff,
                HttpMsg& oHttpMsg)
{
    LOG4_TRACE("%s()", __FUNCTION__);
...
const std::string& CodecWebSocketJson::ToString(const HttpMsg& oHttpMsg)
{
    LOG4_TRACE("%s()", __FUNCTION__);
...
E_CODEC_STATUS CodecWebSocketJson::Decode(tagConnectionAttr* pConn,MsgHead& oMsgHead, MsgBody& oMsgBody)
{
    if (eConnectStatus_init == pConn->ucConnectStatus)//必须以http请求初始化握手协议，否则不是WebSocketJson协议
    {
        if (pConn->pRecvBuff->ReadableBytes() >= 5)//目前只支持Get post 的初始化握手协议
        {
                            ...
                            || memcmp(pReadAddr, "POST ", 5) == 0)
            {
                ...
                if (CODEC_STATUS_OK == eCodecStatus)
                {
                    ...
                    {
                        LOG4_DEBUG("%s()　CodecWebSocketJson　need to init connect status with http head Upgrade for websocket",__FUNCTION__);
                        return CODEC_STATUS_ERR;
                    }
                }
                return eCodecStatus;
        ...
        LOG4_DEBUG("%s()　CodecWebSocketJson　need to init connect status with http post or get request",__FUNCTION__);
        return CODEC_STATUS_ERR;
    }
                ...
                MsgHead& oMsgHead, MsgBody& oMsgBody)
{
    if (pBuff->ReadableBytes() >= 2)//处理websocket
    {
        ...
        if(pBuff->ReadableBytes() < uiHeadSize)
        {
            ...
            LOG4_DEBUG("ReadableBytes(%u) < uiHeadSize(%u),strPayload(%s),wait for data",
...
//        oMsgHead.set_msgbody_len(stMsgHead.body_len);
        ...
        if (uiHeadSize + stMsgHead.body_len > uiPayload)      //
        {
            ...
            strPayload.resize(pBuff->ReadableBytes());
            strPayload.assign(pBuff->GetRawReadBuffer(), pBuff->ReadableBytes());
            LOG4_DEBUG("uiHeadSize(%u) + stMsgHead.body_len(%u) > uiPayload(%u),strPayload(%s)",
        ...
        {
            LOG4_DEBUG("ReadableBytes(%u) < body_len(%u),wait for body data,uiHeadSize(%u)",
        ...
        else    // 有压缩或加密，先解密再解压，然后用MsgBody反序列化
        {
            ...
            LOG4_DEBUG("decode oMsgBody:%s",oMsgBody.DebugString().c_str());
            return (CODEC_STATUS_OK);
        }
    }
    else
