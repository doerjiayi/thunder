...
public:
    void InitElection(const util::CJsonObject& oBeacon);
     ...
     * @return node_id
     */
    uint16 AddNode(const util::CJsonObject& oNodeInfo);
    ...
    void GetBeacon(util::CJsonObject& oBeacon) const;
...
protected:
    void AddNodeBroadcast(const util::CJsonObject& oNodeInfo);
    void RemoveNodeBroadcast(const util::CJsonObject& oNodeInfo);
    void CheckLeader();
    void SendBeaconBeat();
...
private:
    ...
    std::unordered_map<std::string, std::unordered_map<std::string, util::CJsonObject> > m_mapNode;  ///< map<node_type, map<node_identify, util::CJsonObject> >
